...
pt_data |>
  ...
  mutate(
    city = case_when(
      # hk washington dc are tricky examples
  ...
  slice_max(year) |>
  ungroup() |>
  # for hong kong, we need to switch the country code to match the city db
  mutate(country_iso2 =
    case_match(city, "Hong Kong" ~ "HK", .default = country_iso2)) ->
pt_tidy
```
...
pt_data |>
  ...
  separate_wider_delim(Cities, "_", names = c("country_iso2", "city"),
    too_many = "merge") |>
